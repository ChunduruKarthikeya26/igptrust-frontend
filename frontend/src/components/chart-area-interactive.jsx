...
import { getAllSitesAnalytics } from "@/api/consents"
import {
  Select,
  SelectContent,
  SelectItem,
...
const chartConfig = {
  ...
  accepted: {
    label: "Accepted All",
    color: "oklch(0.627 0.265 137.0)", // green
  },
  rejected: {
    label: "Rejected All",
    color: "oklch(0.608 0.211 27.325)", // red
  }
}
...
export function ChartAreaInteractive() {
  ...
  React.useEffect(() => {
    ...
    setIsLoading(true)
    getAllSitesAnalytics(days)
      .then(res => {
        // Map trend fields to chart format:
        // backend returns: { date, total, accepted, rejected, customized }
        // We will show accepted and rejected as stacked areas
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        <CardTitle>Total Consents</CardTitle>
        <CardDescription>
          <span className="hidden @[540px]/card:block">
            Consent activity trend overview
          </span>
          <span className="@[540px]/card:hidden">Consent activity</span>
        </CardDescription>
        <CardAction>
          <ToggleGroup
            type="single"
